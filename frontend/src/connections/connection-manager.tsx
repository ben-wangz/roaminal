...
import { createDefinition, deleteDefinition, duplicateDefinition, generateKey, loadDefinitions, loadKeys, publicKey, updateDefinition, type ConnectionDefinition, type ConfigSource, type DefinitionCollection, type GenerationRequest, type SSHKey } from './connection-api';

type Props = {
  instances: ConnectionInstanceSummary[];
  onConnect: (definitionId: string, reuseFrom?: string) => Promise<void>;
  onGenerated: (instance: ConnectionInstanceSummary) => Promise<void>;
  onOpenWorkspace: () => void;
  onToast: (message: string) => void;
};
type Draft = { hostAlias: string; hostName: string; user: string; port: string; identities: string[]; identitiesOnly: string; strictHostKeyChecking: string; userKnownHostsFile: string; serverAliveInterval: string };
type Editor = { mode: 'create' | 'edit'; definition?: ConnectionDefinition } | null;

const emptyDraft: Draft = { hostAlias: '', hostName: '', user: '', port: '', identities: [], identitiesOnly: '', strictHostKeyChecking: '', userKnownHostsFile: '', serverAliveInterval: '' };

function draftFrom(definition?: ConnectionDefinition): Draft {
  if (!definition) return { ...emptyDraft };
  return { hostAlias: definition.hostAlias || '', hostName: definition.hostName || '', user: definition.user || '', port: definition.port ? String(definition.port) : '', identities: [...definition.identityFileNames], identitiesOnly: definition.identitiesOnly || '', strictHostKeyChecking: definition.strictHostKeyChecking || '', userKnownHostsFile: definition.userKnownHostsFile || '', serverAliveInterval: definition.serverAliveInterval ? String(definition.serverAliveInterval) : '' };
}

function bodyFrom(draft: Draft): Record<string, unknown> {
  return { type: 'ssh', hostAlias: draft.hostAlias.trim(), hostName: draft.hostName.trim() || null, user: draft.user.trim() || null, port: draft.port ? Number(draft.port) : null, identityFileNames: draft.identities, identitiesOnly: draft.identitiesOnly || null, strictHostKeyChecking: draft.strictHostKeyChecking || null, userKnownHostsFile: draft.userKnownHostsFile || null, serverAliveInterval: draft.serverAliveInterval ? Number(draft.serverAliveInterval) : null };
}
...
export function ConnectionManager({ instances, onConnect, onGenerated, onOpenWorkspace, onToast }: Props) {
  ...
  function beginEditor(mode: 'create' | 'edit', definition?: ConnectionDefinition) { setEditor({ mode, definition }); setDraft(draftFrom(definition)); }
  ...
  return <section className="connection-manager" aria-label="Connection manager">
    ...
    {tab === 'connections' ? <>
      {definitions && <SourceBand source={definitions.configSource} />}
    ...
    </> : <KeysPanel keys={keys} instances={instances} onGenerate={() => setGeneration({ algorithm: 'ed25519', rsaBits: null, fileName: 'id_ed25519', comment: '' })} onToast={onToast} />}
    ...
    {generation && <GenerationDialog value={generation} busy={busy} onChange={setGeneration} onSubmit={(event) => void startGeneration(event)} onClose={() => setGeneration(null)} />}
  </section>;
}
...
function DefinitionRow({ definition, editable, instances, onConnect, onEdit, onDuplicate, onDelete }: { definition: ConnectionDefinition; editable: boolean; instances: ConnectionInstanceSummary[]; onConnect: (id: string, reuseFrom?: string) => Promise<void>; onEdit: () => void; onDuplicate: () => void; onDelete: () => void }) {
  ...
  return <article className="connection-row"><div className="connection-row-main"><span className="row-icon"><KeyRound size={17} aria-hidden="true" /></span><div className="connection-copy"><strong>{definition.hostAlias}</strong><small>{destination || 'OpenSSH config destination'}</small><small className="row-facts">{definition.identityFileNames.length} managed keys | {definition.hostVerificationAssessment === 'weakened' ? 'weakened trust' : definition.hostVerificationAssessment}</small></div></div><div className="connection-row-status">{definition.warnings.length + definition.advancedDirectiveCount > 0 && <span className="warning-badge"><ShieldAlert size={13} aria-hidden="true" /> {definition.warnings.length + definition.advancedDirectiveCount}</span>}{(!editable || definition.capabilities.edit === false) && <span className="readonly-badge">read-only</span>}</div><div className="connection-row-actions"><button className="icon-button" type="button" onClick={() => void onConnect(definition.connectionDefinitionId)} aria-label={`Connect to ${definition.hostAlias}`} title={`Connect to ${definition.hostAlias}`}><Play size={16} fill="currentColor" aria-hidden="true" /></button>{live.map((instance) => <button className="icon-button reuse-button" key={instance.id} type="button" onClick={() => void onConnect(definition.connectionDefinitionId, instance.id)} aria-label={`Open over existing ${instance.id.slice(0, 8)} transport`} title="Open over existing transport"><ExternalLink size={15} aria-hidden="true" /></button>)}<button className="icon-button" type="button" onClick={onEdit} disabled={!editable || definition.capabilities.edit === false} aria-label={`Edit ${definition.hostAlias}`} title="Edit connection"><Edit3 size={15} aria-hidden="true" /></button><button className="icon-button" type="button" onClick={onDuplicate} disabled={!editable || definition.capabilities.edit === false} aria-label={`Duplicate ${definition.hostAlias}`} title="Duplicate connection"><Copy size={15} aria-hidden="true" /></button><button className="icon-button danger-icon" type="button" onClick={onDelete} disabled={!editable || definition.capabilities.delete === false} aria-label={`Delete ${definition.hostAlias}`} title="Delete connection"><Trash2 size={15} aria-hidden="true" /></button></div></article>;
}

function KeysPanel({ keys, instances, onGenerate, onToast }: { keys: SSHKey[]; instances: ConnectionInstanceSummary[]; onGenerate: () => void; onToast: (message: string) => void }) {
  ...
  return <div className="keys-panel"><div className="manager-toolbar"><div><h2>SSH keys</h2><p className="panel-muted">Private key contents never leave the SSH directory.</p></div><button className="primary" type="button" onClick={onGenerate}><KeyRound size={15} aria-hidden="true" /> Generate key</button></div><div className="key-table" role="table" aria-label="SSH keys"><div className="key-table-head" role="row"><span>Filename</span><span>Algorithm</span><span>Fingerprint</span><span>Usage</span><span /></div>{keys.map((key) => <div className="key-table-row" role="row" key={key.keyId}><span><strong>{key.fileName}</strong><small>{key.readOnly ? 'read-only' : 'writable'} | {key.status}</small></span><span>{key.algorithm}{key.bits ? ` ${key.bits}` : ''}</span><code>{key.fingerprint || 'Unavailable'}</code><span>{instances.filter((instance) => instance.connectionDefinitionId && instance.sourceHostAlias).length ? 'referenced' : '-'}</span><span>{key.publicKeyAvailable && <button className="icon-button" type="button" disabled={copying === key.keyId} onClick={() => void copy(key)} aria-label={`Copy public key ${key.fileName}`} title="Copy public key"><Clipboard size={15} aria-hidden="true" /></button>}</span></div>)}{keys.length === 0 && <div className="manager-empty">No managed Ed25519 or RSA keys detected.</div>}</div></div>;
}
...
function DefinitionEditor({ editor, draft, keys, busy, onDraft, onSave, onClose }: { editor: Exclude<Editor, null>; draft: Draft; keys: SSHKey[]; busy: boolean; onDraft: (draft: Draft) => void; onSave: (event: React.FormEvent) => void; onClose: () => void }) {
  ...
  return <Modal onClose={onClose}><form className="connection-editor" onSubmit={onSave}><header><div><p className="eyebrow">STRUCTURED SSH CONFIG</p><h2>{editor.mode === 'create' ? 'New connection' : `Edit ${editor.definition?.hostAlias || ''}`}</h2></div><button className="icon-button" type="button" onClick={onClose} aria-label="Close editor"><X size={17} aria-hidden="true" /></button></header><label>Host alias<input required pattern="[A-Za-z0-9][A-Za-z0-9._-]{0,254}" value={draft.hostAlias} onChange={(event) => set('hostAlias', event.target.value)} /></label><label>HostName<input value={draft.hostName} onChange={(event) => set('hostName', event.target.value)} placeholder="destination hostname" /></label><div className="form-grid"><label>User<input value={draft.user} onChange={(event) => set('user', event.target.value)} /></label><label>Port<input type="number" min="1" max="65535" value={draft.port} onChange={(event) => set('port', event.target.value)} /></label></div><label>Identity files<div className="identity-options">{keys.map((key) => <label key={key.keyId}><input type="checkbox" checked={draft.identities.includes(key.fileName)} onChange={(event) => onDraft({ ...draft, identities: event.target.checked ? [...draft.identities, key.fileName] : draft.identities.filter((name) => name !== key.fileName) })} />{key.fileName}</label>)}</div></label><div className="form-grid"><label>IdentitiesOnly<select value={draft.identitiesOnly} onChange={(event) => set('identitiesOnly', event.target.value)}><option value="">Unset</option><option value="yes">yes</option><option value="no">no</option></select></label><label>ServerAliveInterval<input type="number" min="0" max="4294967295" value={draft.serverAliveInterval} onChange={(event) => set('serverAliveInterval', event.target.value)} /></label></div><div className="form-grid"><label>StrictHostKeyChecking<select value={draft.strictHostKeyChecking} onChange={(event) => set('strictHostKeyChecking', event.target.value)}><option value="">default</option><option value="no">no</option></select></label><label>UserKnownHostsFile<select value={draft.userKnownHostsFile} onChange={(event) => set('userKnownHostsFile', event.target.value)}><option value="">default</option><option value="/dev/null">/dev/null</option></select></label></div>{(draft.strictHostKeyChecking === 'no' || draft.userKnownHostsFile === '/dev/null') && <div className="risk-warning" role="alert"><ShieldAlert size={16} aria-hidden="true" /> Host verification is weakened for this connection.</div>}<footer><button className="text-button" type="button" onClick={onClose}>Cancel</button><button className="primary" type="submit" disabled={busy}>{busy ? 'Saving...' : 'Save connection'}</button></footer></form></Modal>;
}

function GenerationDialog({ value, busy, onChange, onSubmit, onClose }: { value: GenerationRequest; busy: boolean; onChange: (value: GenerationRequest) => void; onSubmit: (event: React.FormEvent) => void; onClose: () => void }) {
  return <Modal onClose={onClose}><form className="connection-editor" onSubmit={onSubmit}><header><div><p className="eyebrow">SSH KEYGEN</p><h2>Generate key</h2></div><button className="icon-button" type="button" onClick={onClose} aria-label="Close key generator"><X size={17} aria-hidden="true" /></button></header><label>Algorithm<select value={value.algorithm} onChange={(event) => onChange({ ...value, algorithm: event.target.value as GenerationRequest['algorithm'], fileName: event.target.value === 'rsa' ? 'id_rsa' : 'id_ed25519', rsaBits: event.target.value === 'rsa' ? 3072 : null })}><option value="ed25519">Ed25519</option><option value="rsa">RSA</option></select></label>{value.algorithm === 'rsa' && <label>RSA bits<select value={value.rsaBits || 3072} onChange={(event) => onChange({ ...value, rsaBits: Number(event.target.value) })}><option value="2048">2048</option><option value="3072">3072</option><option value="4096">4096</option></select></label>}<label>Filename<input required value={value.fileName} onChange={(event) => onChange({ ...value, fileName: event.target.value })} /></label><label>Comment <span className="optional">optional</span><input maxLength={255} value={value.comment} onChange={(event) => onChange({ ...value, comment: event.target.value })} /></label><div className="risk-warning"><KeyRound size={16} aria-hidden="true" /> Passphrase prompts appear only in the new key generation terminal.</div><footer><button className="text-button" type="button" onClick={onClose}>Cancel</button><button className="primary" type="submit" disabled={busy}><Play size={15} aria-hidden="true" /> {busy ? 'Starting...' : 'Open keygen terminal'}</button></footer></form></Modal>;
}
