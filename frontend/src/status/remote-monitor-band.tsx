...
export function RemoteMonitorBand({ instance }: Props) {
  ...
  const remoteInstanceId = instance?.connectionInstanceId;
  useEffect(() => {
    setSnapshot(null);
    setError(false);
    ...
    const poll = async () => {
      ...
      try {
        const next = await remoteMonitor(remoteInstanceId, controller.signal);
        if (!disposed) {
          setSnapshot(next);
          setError(false);
        }
        ...
        if (!disposed && !document.hidden)
          timer = window.setTimeout(() => void poll(), 5000 + Math.floor(Math.random() * 450));
      }
    };
    const onVisibility = () => {
      if (!document.hidden) {
  ...
  return (
    ...
    >
      <div className="remote-monitor-heading">
        ...
        <span className={`remote-monitor-status status-${snapshot?.status || 'warming'}`}>
          {snapshot?.status || 'warming'}
        </span>
        {error && <span className="remote-monitor-error">probe unavailable</span>}
      </div>
      <div className="remote-monitor-grid">
        ...
        <RemoteMetric
          ...
          detail={snapshot?.sampledAt ? 'freshness' : 'waiting'}
        />
        <RemoteMetric
          label="RTT"
...
function RemoteMetric({ label, value, detail }: { label: string; value: string; detail: string }) {
  return (
    <span className="remote-monitor-metric" title={`${label}: ${value} (${detail})`}>
      <b>{label}</b>
      <strong>{value}</strong>
      <small>{detail}</small>
    </span>
...
function formatLoad(load: RemoteMonitorSnapshot['metrics']['load'] | undefined): string {
