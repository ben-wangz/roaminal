...
export function ContextualKeyboard({ instance, runtime, mode, onModeChange }: Props) {
  ...
  const enabled = Boolean(
    runtime && !runtime.closedState() && runtime.connectedState() && instance?.lifecycle === 'live',
  );
  const disabledReason = !runtime
    ? 'No active terminal'
  ...
  return (
    <section className="contextual-keyboard" aria-label="Virtual keyboard">
      ...
      {!enabled && (
        <span className="contextual-keyboard-status" role="status">
          {disabledReason}
        </span>
      )}
