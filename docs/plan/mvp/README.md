...
部署方式和验收方法。`DEC-001` 至 `DEC-033` 已全部确认，不存在待实施 Agent
选择的分支。
...
1. 先按阅读顺序读完 README 和 9 个实施模块，再从 Phase 0 开始；
   `10-rationale.md` 仅在需要理解取舍时阅读。
...
先读完 README 和 01 至 09 模块，再从 Phase 0 连续执行到所有
