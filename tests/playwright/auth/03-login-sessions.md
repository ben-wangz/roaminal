...
1. Log in from contexts A and B with distinct user agents. In A, open Settings
   and select the `Sessions` section.
2. The inline `Login sessions` panel lists both entries, identifies `This browser`
   and `Other browser`, shows a last-seen time and a shortened session ID, and
   never exposes refresh-token hashes or token values.
...
4. Log B in again. From A choose `Log out other sessions` in the Sessions
   section. Only A remains and B loses access as above.
...
6. Switch away from and back to the Sessions section. The list reloads without
   mutating any sessions, and browser Back/Forward does not create duplicates.
