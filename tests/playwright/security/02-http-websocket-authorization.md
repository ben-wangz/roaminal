...
1. Without auth, verify `/healthz`, `/api/v2/version`, and login challenge endpoints
   have their documented public behavior. Representative protected routes from
   every current family return `401` JSON errors: login sessions and heartbeat;
...
4. Open an instance WebSocket at `/ws/v2/connection-instances/<id>` with both
   `roaminal.v2` and `roaminal.auth.<current-token>`. The server selects only
   `roaminal.v2` and never echoes the auth-bearing subprotocol. Repeat the same
   contract for a disposable launch at `/ws/v2/connection-launches/<launchId>`.
   Missing, malformed, expired, or revoked auth is rejected before attach.
...
6. Attempt to attach another login session to a pending launch owned by the first
   login session. It is forbidden. A published normal connection remains shareable
