...
export function TerminalActions({ session, canCloseTab, onRename, onAutomaticTitle, onCloseTab, onTerminate }: Props) {
  ...
  function focusItem(position: number) {
    ...
    if (items.length) items[(position + items.length) % items.length]?.focus();
  }
  ...
  return <div className="terminal-actions" ref={root}>
    <button ref={trigger} className="tab-menu-trigger icon-button" type="button" aria-label="Terminal actions" title="Terminal actions" aria-haspopup="menu" aria-expanded={open} aria-controls={menuId} onClick={(event) => { event.stopPropagation(); const rect = event.currentTarget.getBoundingClientRect(); setPosition({ top: rect.bottom + 4, left: Math.max(8, rect.right - 190) }); if (open) closeMenu(true); else setOpen(true); }}>⋮</button>
