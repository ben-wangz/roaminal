export type SessionSummary = { id: string; createdAt: string; updatedAt: string; shell: string; initialCwd: string; title: string; cwd: string; cols: number; rows: number; closed: boolean; exitStatus: { exitCode: number | null; signal: number | null } | null };
export type ServerMessage =
  | { type: 'snapshot'; data: string }
  | { type: 'meta'; title: string; cwd: string; cols: number; rows: number }
